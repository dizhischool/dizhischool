...
var Outside = {
	...
	getMaxPopulation: function() {
		return $SM.get('game.buildings["hut"]', true) * 4 + $SM.get('game.buildings["bighut"]', true) * 6;
	},
	...
	killMaterial: function(type) {
		var types = 'stores.' + type;
		var x = 30;
		var y = 5;
		var rand = parseInt(Math.random() * (x - y + 1) + y);
		var rand = rand / 100;
		var oldMat = $SM.get(types, true);
		var KillMat = Math.round(oldMat * rand);
		var newMat = Math.round(oldMat - KillMat);
		$SM.set(types, newMat);
		Notifications.notify(null, _('{0} {1} lost', KillMat, _(type)));
	},
	...
	checkWorker: function(name) {
		var jobMap = {
			'lodge': ['hunter', 'trapper', 'lode runner'],
			'tannery': ['tanner'],
