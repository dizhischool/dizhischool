...
var MainManager = function() {
		...
		function t() {
			g = 1, $("#scene_loading hr").css("display", "none"), $("#scene_loading hr").css("width", 0), $("#scene_loading").stop()
				.fadeOut(200, "linear"), v ? ($("#scene_loading").stop().css("display", "none"), $("#bt_back").stop().css("display",
					"none"), p && $("#bt_fs").stop().css("display", "none"), $("#scene_main .set").stop().css("display", "none")) : $(
					"#scene_main").stop().fadeIn(200, "linear"), I = aidn.___waContext.currentTime, k.start(), P.start()
		}

		function i(a) {
			(X = !X) ? ($("#bt_backtrack a").text("背景音乐: 开启"), aidn.util.setCookie("bt", "on", 2592e3)) : ($("#bt_backtrack a").text(
				"背景音乐: 关闭"), aidn.util.setCookie("bt", "off", 2592e3)), a && a.preventDefault()
		}
		...
		function o() {
			if (P.update(), 1 == g && 0 > --L && r(), v && 1 == g) {
				...
				if (l * s < a) {
					...
					if (h += e - l, 0 <= (l = e) * s - a)
						for (e = Math.random(), a = 1, 192 <= h ? h = 0 : 128 <= h ? (.7 > e && (a = 2), .5 > e && (a = 3)) : 64 <= h ? (
								.35 > e && (a = 2), .2 > e && (a = 3), .02 > e && (a = 0)) : 32 <= h ? (.35 > e && (a = 2), .24 > e && (a = 0)) :
							0 <= h && .4 > e && (a = 0), e = 0; e < a; e++) d = c[f], 32 <= ++f && (f = 0, aidn.util.shuffleArray(c)), k.changeId(
							d, 0, !0)
				}
			}
		...
		function r() {
			v || _ || (_ = !0, $("#bt_back").stop().fadeIn(200, "linear"), p && $("#bt_fs").stop().fadeIn(200, "linear"), $(
		...
		aidn.util.useDummyDiv(), this.init = function() {
			...
			2 <= window.devicePixelRatio && (e = 2), (x = PIXI.autoDetectRenderer(w, M, {
				...
				antialias: !1,
				resolution: e
			})).autoResize = !0, document.getElementById("view").appendChild(x.view), b = new PIXI.Container, k.init(), a(), $(
				"#scene_top").fadeIn(300), o()
		};
		...
		p && ($("#bt_fs").css("display", "block"), $("#bt_fs").click(function(a) {
			...
			$("#scene_top").stop().fadeOut(200, "linear"), $("#scene_loading").stop().fadeIn(200, "linear"), 2 == C ? t() : ((
				new aidn.WebAudio).load(""), P.init(n, e)), a.preventDefault()
			...
			switch (g) {
				case 1:
					g = 0, k.end(), P.end(), $("#scene_top").stop().fadeIn(100, "linear"), $("#scene_loading").stop().fadeOut(100,
						"linear"), $("#scene_main").stop().fadeOut(100, "linear"), r();
					break;
				default:
		...
		var w, M, I, x, b, C = 0,
			...
			T = new function() {
				...
				for (l[6] = .08, l[20] = .1, l[23] = .1, r[1] = 1.3, r[2] = 1.6, r[3] = 1.35, r[5] = 1.7, r[9] = .8, r[17] = .8, r[
						22] = .9, r[25] = .7, r[29] = 1.2, s = 0; s < o; s++) r[s] *= 1.2;
				var d, h, c = 6e4 / 280
			},
			k = new function() {
				function a(a, e) {
				...
				function t(e) {
					...
					if (r(n), e.originalEvent && e.originalEvent.touches)
						for (var t = e.originalEvent.touches.length, i = 1; i < t; i++) n = e.originalEvent.touches[i], n = a(n.pageX, n.pageY),
							r(n, 1)
				}
				...
				function r(a, e, n) {
					v != a && (1 != e && (v = a), 0 > v || (T.play(a % T.length, n), L = 90, _ && (_ = !1, $("#bt_back").stop().fadeOut(
						200, "linear"), p && $("#bt_fs").stop().fadeOut(200, "linear"), $("#scene_main .set").stop().fadeOut(200,
						"linear")), 0 >= --F && (e = Math.floor(k.length * Math.random()), n = e + P.length, (e = G[n].length ? G[n].pop() :
						new k[e](X, n)).play(), F = 12 * Math.random() + 6), e = a % P.length, (e = 0 < G[e].length ? G[e].pop() : new P[
						e](X, e)).play()))
				}
				...
				var d = function(a, e) {
					...
					h = function(a, e) {
						function n() {
							...
							var e, n;
							if (0 == o)
								for (var t = 0; t < h.rotation; t += 30) e = (s * t + l) * Math.PI / 180, n = Math.cos(e) * a, e = Math.sin(e) *
									a, c.lineTo(n, e);
							else
								for (t = 360; h.rotation < t; t -= 30) e = (s * t + l) * Math.PI / 180, n = Math.cos(e) * a, e = Math.sin(e) *
									a, c.lineTo(n, e);
							e = (s * h.rotation + l) * Math.PI / 180, n = Math.cos(e) * a, e = Math.sin(e) * a, c.lineTo(n, e), c.lineTo(0,
								0), c.endFill()
						}
						...
						this.play = function(a, e) {
							o = 0, d = a, r = e, l = 360 * Math.random(), s = 1, .5 > Math.random() && (s = -1), c.clear(), c.beginFill(0),
								c.moveTo(0, 0), c.lineTo(1, 1), c.endFill(), TweenLite.fromTo(h, .6, {
									rotation: 0
								}, {
									rotation: 360,
									ease: Power1.easeOut,
					...
					c = function(a) {
						...
						this.play = function(a, f) {
							...
							s = Math.floor(5 * Math.random()) + 3, r = 5 * Math.random() + 3, h.clear(), h.rotation = 30 * Math.floor(6 *
								Math.random()), c = {};
							var v;
							v = 0 == i ? 3 : 2.5;
					...
					f = function(a, e) {
						...
						var t = function(a) {
							...
							this.play = function(a, f, u, v) {
								return t.visible = !0, c = 0, i = a, o = f, l = u, s = v, d = .2 * Math.random() + .2, h = .2 * Math.random() +
									.2, r = {
										x: i.x,
										y: i.y
									}, TweenLite.to(r, d, {
						...
						this.play = function(a) {
							r = a, o.setChildIndex(s, o.children.length - 1), s.visible = !0, s.x = w / 2, s.y = M / 2, s.rotation = .5 *
								Math.PI * Math.floor(4 * Math.random()), a = Math.floor(7 * Math.random() + 2);
							var e = .8 * Math.min(w, M);
							i.size = e;
							for (var h, c = e / a * (.4 * Math.random() + .7), f = e / a * (.4 * Math.random() + .1), u = l(), v = 0, p = 0; p <=
								a; p++) {
								var m = {
										x: -e / 2,
										y: h = (p - .5 * a) * c
				...
				this.resize = function() {
					...
					m || ($("#view").on("mousedown", t), $(window).on("mousemove", i), $(window).on("mouseup", o), $(window).on(
						"keydown", e), $(window).on("keyup", n)), (m || window.TouchEvent) && ($("#view").on("touchstart", t), $(window)
						.on("touchmove", i), $(window).on("touchend", o)), $("#view").css("cursor", "pointer")
				}, this.end = function() {
					m || ($("#view").off("mousedown", t), $(window).off("mousemove", i), $(window).off("mouseup", o), $(window).off(
						"keydown", e), $(window).off("keyup", n)), (m || window.TouchEvent) && ($("#view").off("touchstart", t), $(
						window).off("touchmove", i), $(window).off("touchend", o)), $("#view").css("cursor", "auto")
				...
				var u, v = -1,
					...
					P = [function(a, e) {
						...
						var t = function(a) {
							...
							this.play = function(a, t, i) {
								...
								return n.lineStyle(3 * Math.random() + 3, i), n.drawCircle(0, 0, l), n.x = a, n.y = t, n.scale.x = 0, n.scale
									.y = 0, n.rotation = Math.random() * Math.PI, a = .2 * Math.random() + .4, TweenLite.to(n, a, {
										x: o,
										y: r,
						...
						var t = function(a) {
							...
							this.play = function(a, t, i) {
								...
								return n.beginFill(i), n.drawRect(-l / 2, -l / 2, l, l), n.x = a, n.y = t, n.scale.x = 0, n.scale.y = 0, n.rotation =
									Math.random() * Math.PI, a = .2 * Math.random() + .4, TweenLite.to(n, a, {
										x: o,
										y: r,
										rotation: Math.random() * Math.PI,
										ease: Power3.easeOut,
						...
						this.play = function() {
							o.setChildIndex(s, o.children.length - 1), s.visible = !0, s.x = w / 2, s.y = M / 2, s.rotation = Math.random() *
								Math.PI * 2;
						...
						var t = function(a) {
							function e() {
								...
								r.lineStyle(4 * Math.random() + 4, l()), r.drawRect(-e / 2, -e / 2, e, e), r.x = i + a / 2 * (Math.random() -
										.5), r.y = o + a / 2 * (Math.random() - .5), r.scale.x = 0, r.scale.y = 0, r.rotation = Math.random() *
									Math.PI, TweenLite.to(r, .5, {
										x: i,
										y: o,
										rotation: 0,
										ease: Back.easeOut.config(1.7),
						...
						var t = function(a) {
							function e() {
								...
								r.beginFill(l()), r.drawCircle(0, 0, a), r.x = i, r.y = o, r.scale.x = 0, r.scale.y = 0, TweenLite.to(r.scale,
									.5, {
										x: 1,
										y: 1,
										ease: Elastic.easeOut.config(1, .3),
						...
						this.play = function() {
							...
							var e = (a = 45 * Math.PI / 180 * Math.floor(2 * Math.random())) + 45 * Math.PI / 180 * Math.floor(4 * Math.random() -
									2),
								t = .3 * Math.random() + 1,
								s = .3 * -Math.random() + 1;
						...
						this.play = function() {
							i.setChildIndex(o, i.children.length - 1), o.clear(), o.visible = !0, o.lineStyle(5 * Math.random() + 3, l(),
								1), o.x = w / 2, o.y = M / 2;
							for (var a = .6 * Math.min(w, M), e = Math.floor(5 * Math.random()) + 3, t = 360 / e, r = .5 * Math.max(w, M) /
									a * (1.6 + .6 / e), s = 0; s <= e; s++) {
						...
						var t = function(a) {
							...
							this.init = function(a, e, n, t) {
								...
								t = n, i.clear(), i.visibloe = !0, i.beginFill(l), i.drawRect(.5 * -o, .5 * -o, o, o), TweenLite.fromTo(i.scale,
									.3, {
										x: 0,
										y: 0
									}, {
						...
						this.play = function() {
							o.visible = !0, i.setChildIndex(o, i.children.length - 1), o.x = .2 * w + .6 * w * Math.random(), o.y = .2 * M +
								.6 * M * Math.random();
							var a = Math.min(w, M) * (.7 + .2 * Math.random()),
								e = a / 10 * (.5 + .8 * Math.random()),
								t = l();
							r.clear(), r.beginFill(t), r.drawRect(0, -e / 2, a, e), s.clear(), s.beginFill(t), s.drawRect(-e / 2, 0, e, a),
								r.y = 0, r.x = -a / 2, s.x = 0, s.y = -a / 2, o.rotation = 45 * Math.PI / 180, r.scale.x = 0, s.scale.y = 0,
								a = .5 > Math.random() ? -135 * Math.PI / 180 : 215 * Math.PI / 180, (new TimelineLite).to(r.scale, .4, {
									x: 1,
									ease: Power2.easeOut
								}).to(s.scale, .4, {
									y: 1,
						...
						this.play = function() {
							f.clear(), f.visible = !0, .5 > Math.random() ? (f.x = 0, f.y = 0, f.rotation = 0) : (f.x = w, f.y = M, f.rotation =
								Math.PI), h = o = 0, i = Math.floor(3 * Math.random()) + 3, r = 20 * Math.random() + 2, s = l();
					...
					k = [function(a, e) {
						...
						this.play = function() {
							D = c;
							var a = s();
							r = E[a], $("#about").css("background-color", "#" + r.toString(16)), .3 > Math.random() && z.flash(d), A = a,
								h.clear(), h.visible = !0, d = o.children.length - 1 - Math.floor(2 * Math.random()), o.setChildIndex(h, d);
							a = .5 > Math.random();
							...
							for (var u = l = 0; u <= e; u++) {
								var v = {
									...
									p = 0;
								0 != u && u != e && (p = f / 4 * Math.random() - f / 8), a ? v.y = f * u + p : v.x = f * u + p, i.pos["p" + u] =
									v, v = .4 * Math.random() + .3, l = 2, TweenLite.to(i.pos["p" + u], v, a ? {
										x: w
									} : {
										y: M
									})
				...
				var z, E = [13430510, 8965324, 9099756, 961181, 1089457, 34969, 13934238, 16110792, 15488645, 16531063, 5853015,
						3222317
					],
		...
		i(), aidn.util.webaudio ? ($("#ng").css("display", "none"), $(".ok").css("display", "block"), m && $(
				"#scene_main .attention").html("轻触 &amp; 滑动!"), u || $("#scene_top .attention").text("* 把声音开大点，然后享受吧！")) : ($("#ng")
				.css("display", "block"), $(".ok").css("display", "none"), u || $("#ng .atten").html("抱歉，</br>你的浏览器不支此页面")), PIXI.utils
			._saidHello = !0, aidn.window.resize(a)
	},
